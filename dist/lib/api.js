...
const logout = (req, res) => {
  req.logOut(err => {
    if (err) {
      return (0, _util.failure)(res, new Error(err));
    }
...
const sendSessionUser = (req, res) => (0, _util.success)(res, req.user);

exports.sendSessionUser = sendSessionUser;

const sendTrainingData = (req, res) => (0, _util.success)(res, _data.trainingData);

exports.sendTrainingData = sendTrainingData;

const sendBookingData = (req, res) => (0, _util.success)(res, _data.bookings);
...
const getBookingsByTrainingId = (req, res) => {
  ...
  if (!_data.trainingData[id]) {
    return (0, _util.failure)(res, (0, _errors.InvalidId)(id), 500);
  }

  return (0, _util.success)(res, filtered);
};
...
const bookTimeSlotForTraining = (req, res) => {
  ...
  if (!(0, _util.verifyTimeSlotParam)(timeSlot)) {
    return (0, _util.failure)(res, (0, _errors.MalformedInput)('timeSlot'), 500);
  }
  ...
  if (filtered.length === training.slots) {
    return (0, _util.failure)(res, _errors.BookedOut, 401);
  }
  ...
  return (0, _util.success)(res, booking);
};
...
const cancelTimeSlotForTraining = (req, res) => {
  ...
  if (index === -1) {
    return (0, _util.failure)(res, _errors.RemoveNonExistingItemError);
  }
...
const addTimeSlotToTraining = (req, res) => {
  ...
  if (training.availableTimeSlots.includes(timeSlot)) {
    return (0, _util.failure)(res, _errors.AddDuplicateItemError);
  }

  training.availableTimeSlots.push(timeSlot);
  return (0, _util.success)(res, training);
};
...
const deleteTimeSlotFromTraining = (req, res) => {
  ...
  if (filtered.length > 0) {
    return (0, _util.failure)(res, _errors.RemoveBookedItemError);
  }
